<?php

namespace App\Controller\Core\TransmissionReponse;

use App\Repository\Cour\TransmissionReponseRepository;
use App\Repository\Cour\CourrierInterneRepository;
use Doctrine\ORM\EntityManagerInterface;
use OpenApi\Attributes as OA;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Attribute\Route;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;

#[OA\Tag(name: "TransmissionReponse")]
class InstanceController extends AbstractController
{
    public function __construct(
        private TransmissionReponseRepository $transmissionReponseRepository,
        private CourrierInterneRepository $courrierInterneRepository,
        private EntityManagerInterface $entityManager
    ) {}

    #[Route('/core/transmission-reponse/set-instance', name: 'app_core_transmission_reponse_set_instance', methods: ['PATCH'])]
    #[OA\Patch(
        path: '/core/transmission-reponse/set-instance',
        summary: 'Mettre une ou plusieurs transmissions reponse en instance',
        tags: ['TransmissionReponse'],
        description: 'Met une ou plusieurs transmissions reponse en instance en definissant isinstance a true.',
        security: [["bearerAuth" => []]],
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(
                type: 'object',
                required: ['ids'],
                properties: [
                    new OA\Property(
                        property: 'ids',
                        type: 'array',
                        items: new OA\Items(type: 'integer'),
                        example: [1, 2, 3],
                        description: 'Liste des IDs des transmissions reponse a mettre en instance'
                    )
                ]
            )
        ),
        responses: [
            new OA\Response(
                response: 200,
                description: 'Transmissions reponse mises en instance avec succes.',
                content: new OA\JsonContent(
                    type: 'object',
                    properties: [
                        new OA\Property(property: 'message', type: 'string', example: '3 transmission(s) reponse mise(s) en instance avec succes'),
                        new OA\Property(property: 'total', type: 'integer', example: 3),
                        new OA\Property(property: 'success', type: 'integer', example: 3),
                        new OA\Property(property: 'errors', type: 'integer', example: 0),
                        new OA\Property(
                            property: 'details',
                            type: 'array',
                            items: new OA\Items(
                                type: 'object',
                                properties: [
                                    new OA\Property(property: 'id', type: 'integer', example: 1),
                                    new OA\Property(property: 'status', type: 'string', example: 'success'),
                                    new OA\Property(property: 'isinstance', type: 'boolean', example: true),
                                    new OA\Property(property: 'statut', type: 'string', example: 'Instancie')
                                ]
                            )
                        )
                    ]
                )
            ),
            new OA\Response(response: 400, description: 'Requete invalide (IDs manquants ou format incorrect).')
        ]
    )]
    public function setInstance(Request $request): Response
    {
        $data = json_decode($request->getContent(), true);

        if (!isset($data['ids']) || !is_array($data['ids']) || empty($data['ids'])) {
            return $this->json([
                'code' => 400,
                'message' => 'Le champ "ids" est requis et doit etre un tableau non vide.'
            ], 400);
        }

        $ids = $data['ids'];
        $results = [];
        $successCount = 0;
        $errorCount = 0;

        foreach ($ids as $id) {
            if (!is_numeric($id)) {
                $results[] = [
                    'id' => $id,
                    'status' => 'error',
                    'message' => 'ID invalide (doit etre un nombre entier).'
                ];
                $errorCount++;
                continue;
            }

            $transmissionReponse = $this->transmissionReponseRepository->find((int) $id);

            if (!$transmissionReponse) {
                $results[] = [
                    'id' => $id,
                    'status' => 'error',
                    'message' => 'Transmission reponse non trouvee.'
                ];
                $errorCount++;
                continue;
            }

            if ($transmissionReponse->isinstance()) {
                $results[] = [
                    'id' => $id,
                    'status' => 'warning',
                    'message' => 'La transmission reponse est deja en instance.',
                    'isinstance' => true
                ];
                $errorCount++;
                continue;
            }

            try {
                $transmissionReponse->setInstance(true);
                $linkedCourrierInterneIds = $transmissionReponse->getIdCourrierInternes();
                if (is_array($linkedCourrierInterneIds)) {
                    foreach ($linkedCourrierInterneIds as $courrierInterneId) {
                        if (!is_numeric($courrierInterneId)) {
                            continue;
                        }
                        $linkedCourrierInterne = $this->courrierInterneRepository->find((int) $courrierInterneId);
                        if ($linkedCourrierInterne && !$linkedCourrierInterne->isinstance()) {
                            $linkedCourrierInterne->setInstance(true);
                            $linkedCourrierInterne->refreshStatut();
                        }
                    }
                }
                $transmissionReponse->refreshStatut();

                $results[] = [
                    'id' => $transmissionReponse->getId(),
                    'status' => 'success',
                    'isinstance' => true,
                    'statut' => $transmissionReponse->getStatut(),
                    'updatedAt' => $transmissionReponse->getUpdatedAt()?->format('c')
                ];
                $successCount++;
            } catch (\Exception $e) {
                $results[] = [
                    'id' => $id,
                    'status' => 'error',
                    'message' => 'Erreur lors de la mise en instance : ' . $e->getMessage()
                ];
                $errorCount++;
            }
        }

        try {
            $this->entityManager->flush();
        } catch (\Exception $e) {
            return $this->json([
                'code' => 500,
                'message' => 'Erreur lors de la sauvegarde : ' . $e->getMessage()
            ], 500);
        }

        return $this->json([
            'message' => sprintf(
                '%d transmission(s) reponse mise(s) en instance avec succes%s',
                $successCount,
                $errorCount > 0 ? sprintf(', %d erreur(s)', $errorCount) : ''
            ),
            'total' => count($ids),
            'success' => $successCount,
            'errors' => $errorCount,
            'details' => $results
        ], 200);
    }

    #[Route('/core/transmission-reponse/unset-instance', name: 'app_core_transmission_reponse_unset_instance', methods: ['PATCH'])]
    #[OA\Patch(
        path: '/core/transmission-reponse/unset-instance',
        summary: 'Desinstancier une ou plusieurs transmissions reponse',
        tags: ['TransmissionReponse'],
        description: 'Annule l\'instance d\'une ou plusieurs transmissions reponse en definissant isinstance a false.',
        security: [["bearerAuth" => []]],
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(
                type: 'object',
                required: ['ids'],
                properties: [
                    new OA\Property(
                        property: 'ids',
                        type: 'array',
                        items: new OA\Items(type: 'integer'),
                        example: [1, 2, 3],
                        description: 'Liste des IDs des transmissions reponse a desinstancier'
                    )
                ]
            )
        ),
        responses: [
            new OA\Response(
                response: 200,
                description: 'Instances des transmissions reponse annulees avec succes.',
                content: new OA\JsonContent(
                    type: 'object',
                    properties: [
                        new OA\Property(property: 'message', type: 'string', example: '3 instance(s) annulee(s) avec succes'),
                        new OA\Property(property: 'total', type: 'integer', example: 3),
                        new OA\Property(property: 'success', type: 'integer', example: 3),
                        new OA\Property(property: 'errors', type: 'integer', example: 0),
                        new OA\Property(
                            property: 'details',
                            type: 'array',
                            items: new OA\Items(
                                type: 'object',
                                properties: [
                                    new OA\Property(property: 'id', type: 'integer', example: 1),
                                    new OA\Property(property: 'status', type: 'string', example: 'success'),
                                    new OA\Property(property: 'isinstance', type: 'boolean', example: false),
                                    new OA\Property(property: 'statut', type: 'string', example: 'Transmis')
                                ]
                            )
                        )
                    ]
                )
            ),
            new OA\Response(response: 400, description: 'Requete invalide (IDs manquants ou format incorrect).')
        ]
    )]
    public function unsetInstance(Request $request): Response
    {
        $data = json_decode($request->getContent(), true);

        if (!isset($data['ids']) || !is_array($data['ids']) || empty($data['ids'])) {
            return $this->json([
                'code' => 400,
                'message' => 'Le champ "ids" est requis et doit etre un tableau non vide.'
            ], 400);
        }

        $ids = $data['ids'];
        $results = [];
        $successCount = 0;
        $errorCount = 0;

        foreach ($ids as $id) {
            if (!is_numeric($id)) {
                $results[] = [
                    'id' => $id,
                    'status' => 'error',
                    'message' => 'ID invalide (doit etre un nombre entier).'
                ];
                $errorCount++;
                continue;
            }

            $transmissionReponse = $this->transmissionReponseRepository->find((int) $id);

            if (!$transmissionReponse) {
                $results[] = [
                    'id' => $id,
                    'status' => 'error',
                    'message' => 'Transmission reponse non trouvee.'
                ];
                $errorCount++;
                continue;
            }

            if (!$transmissionReponse->isinstance()) {
                $results[] = [
                    'id' => $id,
                    'status' => 'warning',
                    'message' => 'La transmission reponse n\'est pas en instance.',
                    'isinstance' => false
                ];
                $errorCount++;
                continue;
            }

            try {
                $transmissionReponse->setInstance(false);
                $linkedCourrierInterneIds = $transmissionReponse->getIdCourrierInternes();
                if (is_array($linkedCourrierInterneIds)) {
                    foreach ($linkedCourrierInterneIds as $courrierInterneId) {
                        if (!is_numeric($courrierInterneId)) {
                            continue;
                        }
                        $linkedCourrierInterne = $this->courrierInterneRepository->find((int) $courrierInterneId);
                        if ($linkedCourrierInterne && $linkedCourrierInterne->isinstance()) {
                            $linkedCourrierInterne->setInstance(false);
                            $linkedCourrierInterne->refreshStatut();
                        }
                    }
                }
                $transmissionReponse->refreshStatut();

                $results[] = [
                    'id' => $transmissionReponse->getId(),
                    'status' => 'success',
                    'isinstance' => false,
                    'statut' => $transmissionReponse->getStatut(),
                    'updatedAt' => $transmissionReponse->getUpdatedAt()?->format('c')
                ];
                $successCount++;
            } catch (\Exception $e) {
                $results[] = [
                    'id' => $id,
                    'status' => 'error',
                    'message' => 'Erreur lors de la desinstanciation : ' . $e->getMessage()
                ];
                $errorCount++;
            }
        }

        try {
            $this->entityManager->flush();
        } catch (\Exception $e) {
            return $this->json([
                'code' => 500,
                'message' => 'Erreur lors de la sauvegarde : ' . $e->getMessage()
            ], 500);
        }

        return $this->json([
            'message' => sprintf(
                '%d instance(s) annulee(s) avec succes%s',
                $successCount,
                $errorCount > 0 ? sprintf(', %d erreur(s)', $errorCount) : ''
            ),
            'total' => count($ids),
            'success' => $successCount,
            'errors' => $errorCount,
            'details' => $results
        ], 200);
    }
}
