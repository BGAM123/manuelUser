...
class GetPendingByServiceController extends AbstractController
{
    ...
    #[Route('/core/transmission/pending/{serviceId<([1-9][0-9]*)>}', name: 'app_core_transmission_get_pending_by_service', methods: ['GET'])]
    #[OA\Get(
        path: '/core/transmission/pending/{serviceId}',
        summary: 'Transmissions en attente pour un service',
        tags: ['Transmission'],
        description: "Retourne toutes les transmissions en attente (sans accusé de réception) pour un service.",
        security: [["bearerAuth" => []]],
        parameters: [
            new OA\Parameter(
                name: 'serviceId',
        ...
        responses: [
            new OA\Response(response: 200, description: 'Liste des transmissions en attente récupérée avec succès.'),
            new OA\Response(response: 404, description: 'Service non trouvé.'),
            new OA\Response(response: 401, description: 'Accès non autorisé.')
        ]
    )]
    public function getPendingByService(Request $request, int $serviceId): Response
    {
        $this->accessChecker->checker($this->getUser(), $this->isGranted('ROLE_USER'), 'GetPendingTransmissions');
        ...
        $queryBuilder = $this->transmissionRepository->createQueryBuilder('t')
            ...
            ->andWhere('t.accuseReception = :accuseReception')
            ->setParameter('service', $service)
            ->setParameter('isDelete', false)
            ->setParameter('accuseReception', false)
