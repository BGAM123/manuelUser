...
class PaginatedCollectionNormalizer implements NormalizerInterface
{
    ...
    public function normalize(mixed $object, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        // Clean nested user items from any legacy hypermedia links.
        if (isset($normalizedData['data']) && is_array($normalizedData['data'])) {
            foreach ($normalizedData['data'] as &$item) {
                if (is_array($item) && array_key_exists('_links', $item)) {
                    unset($item['_links']);
                }
            }
            unset($item);
        ...
        // Pagination HATEOAS links intentionally omitted — API returns only `meta` and `data`.
